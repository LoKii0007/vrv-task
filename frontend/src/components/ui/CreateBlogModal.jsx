...
export default function CreateBlogModal({ open, setOpen }) {
    ...
    const onSubmit = async (data) => {
        try {
            ...
            const res = await axios.post(`http://localhost:3000/api/create-blog`, formData, {
            ...
            <div className="bg-white px-4 pb-4 pt-5 sm:p-6 sm:pb-4">
              <form onSubmit={handleSubmit(onSubmit)} className='flex flex-col md:gap-6 gap-4 justify-center items-center' >
                ...
                <div className="grid grid-cols-2 items-center gap-6 w-full ">
                    ...
                    <button disabled={loading} type='submit' className='border bg-blue-500 text-white px-4 py-2 rounded-md'>{loading ? 'Loading...' : 'Submit'}</button>
                </div>
              </form>
            </div>
